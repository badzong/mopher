...
typedef enum acl_value_type { AV_NULL = 0, AV_CONST, AV_FUNC, AV_ATTR }
    acl_value_type_t;

typedef enum acl_symbol_type { AS_NULL = 0, AS_FUNC, AS_ATTR }
    acl_symbol_type_t;

typedef enum acl_action_type { AA_ERROR = -1, AA_NULL = 0, AA_PASS, AA_BLOCK, AA_DISCARD,
    AA_CONTINUE, AA_DELAY, AA_JUMP } acl_action_type_t;


/*
 * Function pointer types
 */
typedef var_t *(*acl_function_t)(ll_t *args);
typedef var_t *(*acl_attribute_t)(ht_t *attrs);

typedef union acl_callback {
    acl_function_t  ac_function;
    acl_attribute_t ac_attribute;
} acl_callback_t;

typedef struct acl_symbol {
    acl_symbol_type_t    as_type;
    char                *as_name;
    acl_callback_t       as_callback;
} acl_symbol_t;
...
typedef union acl_value_data {
    var_t           *vd_var;
    acl_call_t      *vd_call;
    acl_attribute_t  vd_attr;
} acl_value_data_t;

typedef struct acl_value {
    acl_value_type_t    av_type;
    acl_value_data_t    av_data;
} acl_value_t;
...
int acl_symbol_register(acl_symbol_type_t type, char *name, acl_callback_t callback);
void acl_value_delete(acl_value_t * av);
acl_value_t * acl_value_create(acl_value_type_t type, acl_value_data_t data);
acl_value_t * acl_value_create_attribute(char *name);
...
int acl_init(void);
void acl_clear(void);
var_t * acl_value_eval(acl_value_t *av);
int acl_compare(var_t *v1, var_t *v2, acl_cmp_t ac);
int acl_conditions_eval(ll_t *conditions);
